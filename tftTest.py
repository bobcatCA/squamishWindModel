...
for count, training_label in enumerate(training_labels):
    ...
    y_range_pred = pd.Series(raw_predictions.output.prediction[:, 0, 3].numpy(),
                             index=x_range_pred.index)  # Appears to be format [n, tau, quant]
    y_range_forecast = pd.Series(raw_predictions.output.prediction[:, forecast_n, 3].numpy(),
                                 index=x_range_forecast.index)  # Predicted n steps from present

    y_range = {
        'Measured': data[training_label],
        'Predicted': y_range_pred,
        'Forecast': y_range_forecast
    }
